...
impl Config {
    pub fn new(config_name: &str) -> Self {
        ...
        let debug = read_value("debug", false, &config).expect("debug should be one of: true/false");
        let initial_color = read_value("initial_color", 0xFFFF_FFFF, &config).expect("initial_color should be a 32bit number");
        let play_sound = read_value("play_sound", true, &config).expect("play_sound should be one of: true/false");
